...
Benton [here](https://subsurfacesee.org/). Because adoption of this tool is 
...
## Section 2: To get started (assuming pre-2000s Apple OS with Intel x86 / x64 instruction set):
* No official published instruction set here yet - please contact author for support.

## Section 3: To get started (Apple OS with ARM instruction set):
* No setup or support.
...
8) To run the container with file sharing enabled and interacting with it directly, run this command (which is similar to the initial one, which is docker `run -it nathanbenton/sug`): `docker run --mount type=bind,source=C:\Users\<YOUR_USERNAME>\...,target=/usr/local/cwp_su_all_44R22/data -it  nathanbenton/sug` (see email to Dr. Lorenzo for more details).
...
13) See all TODO notes to understand just how much stuff needs fixing and enhancing for this project.
