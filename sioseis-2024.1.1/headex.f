...
!     The execution phase of SIOSEIS process HEADER, the SEGY trace
!  header changer.
!
!  mod 19 Dec 93 - correct spatial interpolation of REAL variables
!  mod 5 Jan 94 - correct problem when lists have same fno but different
!                  trace numbers
!  mod 12 June 95 - Add clean
!  mod 26 June 95 - Fix incorrect lists when nlists > 2
!  mod 23 Dec 96 - Allow math operations on up to 10 header values.
!  mod 20 June 2001 - Allow multiple header processes and make this reentrant.
!  mod 9 Apr 03 - multiple lists didn't work right (how did it ever work?0
!  mod 10 Apr 03 - Made scr big enough to hold the parameter list!
!                - Read (rddisc) into lscr rather than scr.
!  mod 6 May 03 - Add REV1 PATHNAME
!  mod 12 Jun 03 - Adding another parameter screwed up multiple lists.
!  mod 9 Aug 03 - Giving every trace on multiple shots failed.
!  mod 15 Jun 05 - Do the math in REAL*8
!  mod 28 Jul 06 - Allow unsigned integer numdat & ibuf(58)
!                - Watch out for byte swap when doing unsigned numdat
!  mod 12 Feb 07 - Add parameter SWAP
!  mod 28 Nov 07 - Add NOINC and TRINC
!  mod 7 May 08 - Change fno/lno presets so that if if only one list and fno/lno
!                 are not given, then do all (fno =0, lno = 99999987).
!                - If fno is given, but lno is not, then set lno = fno.
!  mod 14 Apr 14 - add arg cbuff and add B (byte) to "equation"
!                - Add arg dbuf and D (double or 64 bit) to swap
!  mod 30 Aug 19 - remove an ibuf(58) byte swap when <0 (an old XSTAR problem) which
!                  was triggered when the number of sample is > 32K
!  mod 10 Sep 19 - Replace with multiple lists was wrong (thanks Alistair)
!  mod 20 Dec 21 - Bizarre gcc 11.2 SAVE didn't SAVE ireplc1 - perhaps equivalence prob?
!
      INTEGER harsh, mild
...
!  The following gives a IEEE error on the Sun, but the answer is ok.
              fno2(numhdr) = lscr(1)
              lno2(numhdr) = lscr(2)
              ftr2(numhdr) = lscr(3)
              ltr2(numhdr) = lscr(4)
              ...
              ENDDO
          ENDIF
!****     Clean the binary header once and only once
          ...
          ENDIF
!****     Do the SEG-Y Rev 1 stuff once and only once
!****     Assume the binary header is in common
          IF( lunrev1 .NE. 0 ) THEN
!****         The user want to do something with Rev1 Extension records
              nextr = ibhead(153)
   ...
   21                 CALL upcase( cheadr(1), 11 )
                      nextr = nextr + 1
!****                 This will fail if there are spaces after the '(('
...
!****
      ...
      ELSE
          ...
          itrno = lbuf(7)
      ENDIF
!**** if only 1 list and fno is given and lno is not, then lno = fno
      IF( nlists(numhdr) .EQ. 1 .AND. lno1(numhdr) .EQ. 9999987 .AND.
     &    fno1(numhdr) .NE. -1 ) lno1(numhdr) = fno1(numhdr)
!**** if more than 1 list and lno is not given, the lno = fno
...
!      IF( itrno .GT. ltr1(numhdr) .AND. fno1(numhdr) .NE. fno2(numhdr) )
!     &    RETURN
     ...
     &        ispat1(numhdr) .EQ. 0 ) RETURN
          ...
          IF( mlists(numhdr) .LT. nlists(numhdr) ) THEN
              ...
              DO i = 1, 120
                 ...
                 chdr2(numhdr,i) = lscr(13+120+120+120+120+i)
              ENDDO
              DO i = 1, maxrep
                 ireplc2(numhdr,i) = lscr(13+120+120+120+120+120+i)
              ENDDO
              DO i = 1, maxswap
                 iswap2(i) = lscr(13+120+120+120+120+120+maxrep+i)
              ENDDO
          ELSE
   ...
   90     CONTINUE
      ENDIF
!****
!****      CHANGE THOSE HEADER VALUES!!!
!****
!****     Do byte swapping first
!****
      DO i = 1, maxswap, 2
      ...
      ENDDO
!****
!****   type 1 = replc, type 2 = add, type 3 multiply
!****
 ...
 1000 CONTINUE
      nrep = -7
      DO i = 1, maxrep/7
!  ireplc order is: 1=type, 2=index, 3=type, 4=index or constant, 5=operation, 
!                   6=type, 7=index or constant
! jtypes = 1 means INTEGER*2
!          2       INTEGER*4
!          3       REAL
!          4       constant
!          5       REAL*8
...
!*****  byte manipulation is messy because of the segy trace header is a mix of 2 & 4 byte integers
...
!****        I'm seeing some weird rounding when doing the REAL*8 to
!****        REAL*4 conversion.  e.g. 36361526.0 becomes 36361528.0
             IF( jtype1(numhdr) .EQ. 3 ) buf(index1) = dtemp1
!                               .EQ. 4   is a constant and can't exist on the left had side
 ...
 1220        ibuf(i) = 0
      ...
      ENDIF
!****
!****  Save the current trace header in COMMON.  Mainly for XSTAR, but ...
!****
      DO i = 1, numtrhdr
         header(i) = buf(i)
      ENDDO
      CALL ushort2long( ibuf(58), numdat)
! 30 Aug 2019 pch - remove the following  - why was it here?   XSTAR??  That's old!!
!      numdat = ibuf(58)
