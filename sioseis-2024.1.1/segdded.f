...
!
!  Programmer notes:
!  I read the first shot in the edit phase for sevreal reasons: 
! 1)  I think some other edits need things like the sample interval and data length.
! 2)  By doing the SEGY header stuff here, the final execution phase is not
!     burdened with the slop
!  This does cause a headache because either the edit passes some stuff to
!  the execution phase or the edit has to reformat the trace(s).  Therefore,
!  The edit doesn't reformat/demultiplex the data, it passes such info as
!  the number of scans per tape block on to the execution.
!
!   mod 1 Oct 02 - Add parameter LDEOLIST
!   mod 29 Oct 02 - If fcset = 99, then use all channel sets.
!                 - If lcset > ncsets, then lcset = ncsets
!  Mod 19 Mar 03 - Separate the ldeo external header into it's own lun
!  mod 20 Mar 03 - Make fcset 99 the preset.
!  mod 22 Apr 03 - Print the number of channels in each channel set whe fcset=99
!  mod 5 Mar 04 - Add iformat 3 for Hydroscience
!               - Allow Hydrosciences format with a 32 byte file header
!  mod 22 Apr 04 - Allow iformat 4 for SIO-RT.
!                - Allow iformat 5 for Gemetrics file naming FFILEN.sgd
!  mod 4 May 04 - Bad index on byte swap on PC version.
!  mod 28 May 04 - Give error message if multiple parameter lists.
!  mod 27 June 04 - Add parameter DESCALE  (YES/NO, preset NO).
!  mod 27 Sept 04 - Set the sort flag to 1 in the binary header.
!  mod 25 Apr 06 - Bad preset for NSPFILE - was = 100
!  mod 2 Oct 06 - HUH? - Bad preset for NSPFILE - was = 100
!  mod 14 Aug 07 - g95 IAND requires arguments to be same type and kind.
!  mod 16 Apr 08 - Add FORMAT LDEO for using the LDEO external header
!                - Warn if FCSET not given and LCSET is given.
!  mod 21 Apr 08 - Append the LOG file if possible.
!  mod 17 Jul 08 - Add shot_inc (second parameter of parameter STACK)
!                - finish up right if errors
!  mod 18 Apr 11 - Change retrac so that <0 means cummulative trace count,
!                  =0 means use the segd trace number, >0 means use user given.
!  mod 6 Oct 11 - Change "bad" ftr and ltr values into warnings rather than errors.
!  mod 19 Dec 11 - Wait for the STACK file to exist rather than erroring.
!  mod 9 Mar 12 - Close on free the STACK file while waiting so we don't use too many units.
!  mod 13 Aug 12 - Check for LISTPATH file to exist.
!  mod 30 Jan 13 - Use rshift_sio rather than rshift so vacated bits are zeroed.
!  mod 31 Mar 14 - Change DESCALE preset to 1 - DESCALE ON
!  mod 12 Feb 15 - Error if the file doesn't exist.
!  mod 21 Jan 20 - Allow data format 8024
!                - Convert comment lines to start with ! rather than c
!  mod 14 Apr 20 - Check for Rev 2 128 byte "Storage Unit" header at the front of the file
!  mod 16 Apr 20 - Allow FORMAT ARAM which has a bad nsamps count
!  mod 20 Apr 22 - Large extended and external headers could be a problem so use
!                  podiscb rather than rddiscb if large
!                - Create format LG2019 (format 8) for 2019 Langseth cruise with extra byte
!  mod 1 Feb 24 - Create format KNOC - South Korean Oil Company - 
!                 8015 format with data portion byte swapped
!
...
!****            Get the second file 
                     ...
                     CALL getoke1( token, nchars )
                     CLOSE( UNIT=stack )
!****           the following assumes the format parameter was given
!****           prior to the stack parameter
                     IF( iformat .EQ. 2 .AND.
     ...
     &                           token(1:nchars),' does not exist.'
                         ...
                         GOTO 100
                     ENDIF
!****                list will have the unit number of the list and iunit
!****                will have the unit number of the SEGD data file.
                     CALL GETFIL( 2, list, token, istat )
                     OPEN( UNIT=list, FILE=token,
     &                     FORM='FORMATTED', STATUS='OLD')
!****                GET the first segd file from the list
                     CALL rline1(list)
                     CALL getoke1( token, nchars )
                     pathname = token
                     GOTO 100
...
!                     OPEN( UNIT=lunlog, FILE=token, POSITION='APPEND',
                     OPEN( UNIT=lunlog, FILE=token,
     &                     FORM='FORMATTED', STATUS='UNKNOWN')
                     GOTO 100
                 ENDIF
!****            The first call to magtap will assign the tape
  ...
  160                OPEN( UNIT=ldeolist, FILE=token,
     &                     FORM='FORMATTED', STATUS='OLD')
                 ...
                 IF( names(nparam) .EQ. 'FORMAT' ) THEN
                     ...
                     IF( token(1:nchars) .EQ. 'ARAM' ) iformat = 7
                     IF( token(1:nchars) .EQ. 'LG2019' ) iformat = 8
                     IF( token(1:nchars) .EQ. 'KNOC' ) iformat = 9
                 ENDIF
...
!****
     ...
     *      'large.'
      ...
      ENDIF
!****
!****    Open the first shot, build the pathname if necessary.
!****
...
!****
!****   Now do some work!
!****   Read a tape record, which must have a multiple of 32 bytes in it,
!****  because it is the "general header".  Extract PACKED BCD variables.
!****
...
!****  little endian (pc byte order).
      ...
      ENDIF
!**** read the first general header
      storage_hdr = 0    !  let there be no storage header
     ...
     *       IAND(ibuf(2),i15)
      IF( ifmt .EQ. 3336 ) THEN
          IF( iformat .NE. 3 ) THEN
              ...
              iwarn = iwarn + 1
          ELSE
!****         Skip this one if it's HydroScience's SS iformat
              GOTO 300
          ENDIF
      ENDIF
      IF( ifmt .NE. 8015 .AND. ifmt .NE. 8048 .AND. ifmt .NE. 8058 .AND.
     *    ifmt .NE. 8036 .AND. ifmt .NE. 8024 ) THEN
!****     This will be a little hokey.  If we have 2 ASCII spaces in a row
!****     then it's a 128 byte SEGD Storage Header.
          IF( storage_hdr .EQ. 0 ) THEN
              CALL rddiscb( iunit, lbuf, 96, istat )   ! skip the next 96 bytes
              storage_hdr = 1
              GOTO 300      !  try it from bytes 129-
          ELSE
              PRINT *,' ***  ERROR  ***  SIOSEIS does not',
     *                    ' know about SEG-D format ',ifmt
              ierror = ierror + 1
          ENDIF
      ENDIF
  550 CONTINUE
...
!**** get any general headers in addition to the first 2 which are garunteed
      IF( nblks .GT. 0 ) THEN
          nbytes = nblks * 32
...
!****  big problem ahead with large extended headers, use podiscb for now
          nbytes = nextend * 32
          CALL podiscb( iunit, 2, nbytes )
          indx = indx + nextend * 16
      ENDIF
!****
!****    Get the External header
!****
      ...
      ENDIF
!****
!**** get the number of input traces because things like gather might
!**** need them.  This doesn't quite make sense to me.  I'm off two
!**** scans already, so use a magic number.
!****
      index = (1+nblks) * 16
     ...
     *         IAND(ibuf(index+5),i15)
      ...
      ENDDO
!****
!****  Always save the extended header in luntr0 as a circular file
!****  If we're to save all trace 0 in a file, open a file 
!****
      CALL getfil( 1, luntr0, token, istat )
      IF( iformat .EQ. 6 ) CALL getfil( 1, lunldeo, token, istat )
      IF( tr0 .NE. 0 ) THEN
!****   This is the wierdest parameter.  OUTPUT is the one that
!****   uses it, not segdin.
  ...
  900     FORMAT('tr0.',I3.3,'-',I2.2,I2.2)
      ...
      ENDIF
!****
!****  Write the SEGDEX parameters to a disc file and get all the other
!****  parameter list taken care of before building headers
!****
      lbuf(1) = iunit
      lbuf(2) = ffilen
      lbuf(3) = lfilen
...
!****
      IF( ncsets .NE. 1 .AND. ierror .EQ. 0 ) THEN
          IF( fcset + lcset .EQ. 0 ) THEN
              ...
              iwarn = iwarn + 1
          ENDIF
!          IF( retrac .EQ. 0 .AND. fcset + lcset .EQ. 0 ) THEN
!              PRINT *,' ***  WARNING  ***  RETRAC should be used with mu
!     &ltiple channel sets.'
!              iwarn = iwarn + 1
!          ENDIF
      ENDIF
!****
!****    Build the SEGY EBCDIC tape header
!****
      DO 2100 i = 1, 40
