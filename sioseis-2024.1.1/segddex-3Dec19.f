...
c mod 26 Nov 19 - use integer*2 itohex rather than tohex
c               - get rid of Hydroscience's ncsets 165
c
...
c              CALL itohex(iscr(indx),4,token)
...
c     serial will be wrong on little endian computers
...
c     Number of channels set is nibble 1 & 2 of byte 29
     ...
     *         IAND( rshift(iscr(15),8), i15)
      IF( IAND(lprint,16) .NE. 0 ) THEN
          ...
          PRINT *,' General Header #1, ',token(1:64)
      ENDIF
c     If ncsets = hex FF dec 255, then get it from the next general header
c     the 165 is a bug in the Hydrosciebce acquisition system. dec 165 = hex A5 = bin 1010 0101
      IF( ncsets .EQ. 165 ) THEN
          PRINT *,' *** ABORTED ***'
          PRINT *,' ncsets of 165 no longer supported.'
          PRINT *,' Contact phenkart@gmail.com'
c         it's an integer in bytes 4 & 5 in general header #2
c          ncsets = IAND( iscr(18),i15) * 10 +
c     &             IAND( rshift(iscr(19),12), i15)
c          IF( icompt .EQ. 2 .or. icompt .EQ. 4 ) THEN
c****  between fortran and endiannes this sucks.  We did a 16 bit byte swap before
c              idumb(1) = iscr(17)
c              idumb(2) = iscr(18)
c              CALL swap16( idumb, 2 )
c              CALL swap32( ldumb, 1 )
c              ncsets = IAND( ldumb, i15 ) * 10
c              idumb(1) = iscr(19)
c              idumb(2) = iscr(20)
c              CALL swap16( idumb, 2 )
c              ncsets = ncsets + idumb(1)
c          ENDIF
      ENDIF
      IF( ncsets .GT. 0 ) THEN
          nbytes = 32
