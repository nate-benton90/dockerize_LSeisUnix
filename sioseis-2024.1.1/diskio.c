...
/*   mknamec will fail if fortran file name doesn't terminate with a blank */
...
mod ?? 2019 - Schwehr, ange to conform to gcc8
mod Mar 20 - PMODE_ALL was wrong
mod 19 Mar 20 - put tmp files in /tmp
mod 20 Mar 20 - change PMODE_ALL to PMODE_0700 so that only this user can read/write/delete
                so that other users using /tmp can't delete them
mod 2 Jun 20 - put the tmp files backin the current directory since /tmp is NOT
               cleared on boot anymore (my cygwin anyway)
*/
...
#define	PMODE_0700 0700 /* read, write, execute for just this user  */
	...
	printf("getfil, mode= %d\n",*mode);
      ...
      if(*mode < 0 ) {   /* negative mode means the caller specifies the lun */
		if( *lun < 0 || *lun > MAXFDS ){   /* is it a legal lun? */
			...
			exit(0) ; }
		if(*mode == -1 ) {
/*   use the process id (pid) so no other process can access this file  */
			pid = getpid();
			sprintf(name, "sioseis_%dtmp%d", pid,*lun);
			status = creat( name, PMODE_0700 ); /* read and write */
			close(status);
			status = open(name,2); }
		if(*mode == -2) {
			fd[*lun] = *lun;
		...
		return; }
           ...
           *lun = -1;
           ...
           if(*mode == 1 ) {
               pid = getpid();
/*   use the process id (pid) so no other process can access this file  */
               sprintf(name, "sioseis_%dtmp%d\0", pid,*lun) ;   /* create the name sioseis_tmp##  */
			status = creat( name, PMODE_0700 );
               close(status);
               strcpy(fname[*lun],name);
			fd[*lun] = status;
			status = open(name,2); }
           if( *mode == 3 ) {
			strcpy(tname,name);
                ...
                strcpy(fname[*lun],tname);}
           if( *mode == 4 || *mode == 5 ) {
/*   make sure fortran has a blank or null terminator else this may fail  */
			strcpy(tname,name);
...
#if debug
	printf(" rddisc, lun=%d, from byte %d nbytes=%d, status=%d\n",*lun,offset,nbytes,status);
#endif
       if( status < 0 ) {
